...
fn trace(origin: vec3<f32>, direction: vec3<f32>, collision: ptr<function, vec3<f32>>, iterations: ptr<function, u32>, fog: ptr<function, f32>) -> f32 {
    ...
    *collision = position;
    ...
    let light_vec = rotmat_fn() * normalize(vec3<f32>(sin(u.scrollX - 1.6), 3.0, -cos(u.scrollX)));
    return max(0.0, dot(n, light_vec));
}
...
fn main(@location(0) uv: vec2<f32>, @builtin(position) fragCoord: vec4<f32>) -> @location(0) vec4<f32> {
    ...
    let ol: f32 = 0.25;
    return vec4<f32>(
        sqrt(distance(light_vec, collision) / 10.0) * mix(vec3<f32>(occlusion(iterations) * (2.0 - ol) * lightStrength), 2.0 * fogColor, fog),
        1.0
    );
}
